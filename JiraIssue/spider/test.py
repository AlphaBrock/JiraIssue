...
class Logger(object):
    ...
    def __init__(self, logname="jiraIssueSpider.log", loggeNname=None, loglevel="debug"):
        """
            即在终端打印也在文件打印
        """
...
class Jira(object):
        ...
        if kwargs.get("payload"):
            try:
                response = requests.request(kwargs.get("method"), kwargs.get("url"), headers=kwargs.get("headers"),
                                            data=kwargs.get("payload"), allow_redirects=True, verify=False, timeout=60)
    ...
    def login(self):
        ...
        self.Headers["Referer"] = "https://jira.rizhiyi.com:8443/login.jsp"
        self.Headers["Content-Type"] = "application/x-www-form-urlencoded"

        payload = {
            "os_username": "chen.fei",
        ...
        self.Headers["Cookie"] = "atlassian.xsrf.token="+result.cookies["atlassian.xsrf.token"]
    ...
    def getIssueLists(self):
        ...
        print(self.Headers)
        print(self.httpRequests(url=url, method="POST", payload=payload, headers=self.Headers))
